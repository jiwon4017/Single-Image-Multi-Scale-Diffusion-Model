...
# 텍스트 변형
def augment_text(text, num_augmented_texts=10):
    original_text = text.lower()
    augmented_texts = set()
    while len(augmented_texts) < num_augmented_texts:
        new_text = synonym_replacement(text)
        if new_text.lower() != original_text:
            augmented_texts.add(new_text)
        if len(augmented_texts) < num_augmented_texts:
    ...
    return list(augmented_texts)[:num_augmented_texts]
...
def get_average_embedding(texts):
    ...
    for text in texts:
        ...
        text_embedding =clip_model.get_text_embedding(text, template).to(device).half()
        text_embeddings.append(text_embedding)
    text_embeddings = torch.stack(text_embeddings)
    average_embedding = text_embeddings.mean(dim=0)
...
class MultiscaleTrainer(object):
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            for i in range(self.gradient_accumulate_every):
                ...
                loss = self.model(data, s, similarity_text_emb=self.similarity_text_emb, positive = self.positive , negative = self.negative)
                loss_avg += loss.item()
    ...
    def image2image(self, input_folder='', input_file='', mask='', hist_ref_path='', image_name='', start_s=1, custom_t=None, batch_size=16, scale_mul=(1, 1), device=None, use_hist=False, save_unbatched=True, auto_scale=None, mode=None):
        if custom_t is None:
            custom_t = self.ema_model.num_timesteps_ideal
        ...
        if use_hist:
            ...
            orig_sample_0 = Image.open((hist_ref_path + image_name)).convert("RGB")
        ...
        input_img_tensor = (transforms.ToTensor()(input_img) * 2 - 1)
        input_size = torch.tensor(input_img_tensor.shape[1:])
        input_img_batch = input_img_tensor.repeat(batch_size, 1, 1, 1).to(device)
    ...
    def clip_sampling(self, clip_model, text_input, strength, sample_batch_size, custom_t_list=None,
        ...
        augmented_texts = augment_text(text_input, num_augmented_texts=10)
        self.ema_model.avg_text_embedds = get_average_embedding(augmented_texts)
